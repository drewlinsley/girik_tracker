...
# from models.ffhgru_edited import FFhGRUConvDouble, FFhGRUlateral, FFhGRUhiddenadd
# from models.ffhgru3d_edited import FFhGRU3DEdited
from models.ffhgru3d import FFhGRU3D
from models.ffhgru3d_chrono import FFhGRU3DChrono
...
# pf_root = '/media/data_cifs/projects/prj_tracking/downsampled_constrained_red_blue_datasets_128_16_16/14_dist/tfrecords/'
# pf_root = '/media/data_cifs/projects/prj_tracking/downsampled_constrained_red_blue_datasets_16_64_64/14_dist/tfrecords/'
# pf_root = '/media/data_cifs/projects/prj_tracking/downsampled_constrained_red_blue_datasets_32_32_32/25_dist/tfrecords/'
# pf_root = '/media/data_cifs/projects/prj_tracking/downsampled_constrained_red_blue_datasets_32_128_128/25_dist/tfrecords/'
...
len_train_loader = 40000
len_val_loader = 40000
...
def validate(val_loader, model, criterion, device, logiters=None):
    ...
    with torch.no_grad():
        for i, (imgs, target) in enumerate(val_loader):
            ...
            # doubling the target here to account for the flipped batch appended to the original input batch
            imgs_rot=imgs.rot90(1,[3,4])
            imgs=torch.cat([imgs,imgs_rot], dim=0)
...
if __name__ == '__main__':
    ...
    if args.model == 'hgru':
        ...
        model = FFhGRUwithoutGaussian(batch_size=args.batch_size, timesteps=timesteps, filt_size=15, num_iter=15, exp_name=args.name, jacobian_penalty=jacobian_penalty,
                         grad_method=args.algo)
    elif args.model == 'ffhgru3dchrono':
        print("Init model ffhgru3d with chrono initialized 3D I/O gates ", args.algo, 'penalty: ', args.penalty, 'steps: ', timesteps)
        model = FFhGRU3DChrono(batch_size=args.batch_size, timesteps=timesteps, filt_size=15, num_iter=15, exp_name=args.name, jacobian_penalty=jacobian_penalty,
                         grad_method=args.algo)                         
    elif args.model == 'ffhgru3dedited':
        print("Init model ffhgru 3D edited ", args.algo, 'penalty: ', args.penalty, 'steps: ', timesteps)
        model = FFhGRU3DEdited(batch_size=args.batch_size, timesteps=timesteps, filt_size=15, num_iter=15, exp_name=args.name, jacobian_penalty=jacobian_penalty,
                         grad_method=args.algo)
    elif args.model == 'ffhgruconvdouble':
        print("Init model ffhgru with conv3d layer and double hgru ", args.algo, 'penalty: ', args.penalty, 'steps: ', timesteps)
        model = FFhGRUConvDouble(batch_size=args.batch_size, timesteps=timesteps, filt_size=15, num_iter=15, exp_name=args.name, jacobian_penalty=jacobian_penalty,
                         grad_method=args.algo)
    elif args.model == 'ffhgrulateral':
        print("Init model ffhgru with double lateral views, interleaved frames ", args.algo, 'penalty: ', args.penalty, 'steps: ', timesteps)
        model = FFhGRUlateral(batch_size=args.batch_size, timesteps=timesteps, filt_size=15, num_iter=15, exp_name=args.name, jacobian_penalty=jacobian_penalty,
                         grad_method=args.algo)
    elif args.model == 'ffhgruhiddenadd':
        print("Init model ffhgru with double lateral views, interleaved frames ", args.algo, 'penalty: ', args.penalty, 'steps: ', timesteps)
        model = FFhGRUhiddenadd(batch_size=args.batch_size, timesteps=timesteps, filt_size=15, num_iter=15, exp_name=args.name, jacobian_penalty=jacobian_penalty,
                         grad_method=args.algo)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, (imgs, target) in enumerate(train_loader):
            ...
            # rotating and doubling the input dataset
            imgs_rot=imgs.rot90(1,[3,4])
            imgs=torch.cat([imgs,imgs_rot], dim=0)
            target=torch.cat([target,target])
